...
function HelpCenter({lang,go}){
 ...
 const items=[
  ...
  ['Estatística de Jogo e Desempenho','Em Jogos registe adversário, resultado, cinco inicial e minutos. Os eventos são totalmente editáveis: minuto, parte, ação, atleta, zona e nota. Pode criar novas ações personalizadas. Preencha também estatística coletiva e desempenho individual — nota global, técnica, tática, físico, atitude e observações. Um jogo guardado pode ser reaberto e alterado mais tarde.'],
 ...
 const quick=[
  ['Registar estatística de jogo','games'],['Fazer uma convocatória','callups'],['Preparar um treino','training'],['Criar uma animação','board'],['Importar uma jogada','importer'],['Planear a época','planning']
 ]
 const filtered=items.filter(([a,b])=>(a+' '+b).toLowerCase().includes(q.toLowerCase()))
 return <section className="helpCenter">
  <div className="helpHero"><div><small>GUIA DO TREINADOR · V22.1</small><h2>❓ Ajuda Cavadas Manager</h2><p>Guia operacional para encontrar rapidamente cada função da aplicação.</p></div><div className="helpVersion">ÉPOCA 26/27</div></div>
...
function Games({tr,athletes,games,setGames,callups}){
 const baseTypes=['Golo','Assistência','Remate','Remate enquadrado','Remate fora','Defesa GR','Golo sofrido GR','Recuperação','Perda','Interceção','Roubo','Passe decisivo','1x1 ganho','1x1 perdido','Falta cometida','Falta sofrida','Cartão amarelo','Cartão vermelho','Canto','Livre','Entrada 2.º poste','Ligação no pivô','Chegada do ala','Apoio por baixo','Equilíbrio do fixo']
 const empty=()=>({opponent:'',date:'',time:'',location:'',competition:'',goalsFor:0,goalsAgainst:0,notes:'',events:[],callupId:'',starters:[],playerMinutes:{},performance:{},teamStats:{possession:'',shots:'',shotsOnTarget:'',recoveries:'',losses:'',corners:'',fouls:'',setPieces:'',transitions:'',pivotLinks:'',secondPost:'',notes:''},customTypes:[]})
 const [draft,setDraft]=useState(empty()),[event,setEvent]=useState({minute:'',period:'1ª',type:'Golo',playerId:'',zone:'',note:''}),[editingId,setEditingId]=useState(null),[customType,setCustomType]=useState('')
 const types=[...baseTypes,...(draft.customTypes||[]).filter(x=>!baseTypes.includes(x))]
 const c=callups.find(x=>x.id===draft.callupId);const eligible=c?athletes.filter(a=>(c.players||[]).includes(a.id)):athletes
 const addEvent=()=>{if(!event.type)return;setDraft({...draft,events:[...(draft.events||[]),{...event,id:'ev'+Date.now()}]});setEvent({...event,minute:'',playerId:'',zone:'',note:''})}
 const updateEvent=(id,k,v)=>setDraft({...draft,events:(draft.events||[]).map(x=>x.id===id?{...x,[k]:v}:x)})
 const save=()=>{if(!draft.opponent)return alert('Indique o adversário.');const item={...draft,id:editingId||('g'+Date.now())};setGames(editingId?games.map(g=>g.id===editingId?item:g):[...games,item]);setDraft(empty());setEditingId(null)}
 const editGame=g=>{setDraft({...empty(),...g,events:g.events||[],playerMinutes:g.playerMinutes||{},performance:g.performance||{},teamStats:{...empty().teamStats,...(g.teamStats||{})},customTypes:g.customTypes||[]});setEditingId(g.id);window.scrollTo({top:0,behavior:'smooth'})}
 ...
 const setPerf=(id,k,v)=>setDraft({...draft,performance:{...(draft.performance||{}),[id]:{rating:'',technical:'',tactical:'',physical:'',attitude:'',notes:'',...(draft.performance?.[id]||{}),[k]:v}}})
 const setTeam=(k,v)=>setDraft({...draft,teamStats:{...(draft.teamStats||{}),[k]:v}})
 const addCustomType=()=>{const x=customType.trim();if(!x)return;setDraft({...draft,customTypes:[...(draft.customTypes||[]).filter(t=>t!==x),x]});setEvent({...event,type:x});setCustomType('')}
 const counts=(draft.events||[]).reduce((m,e)=>(m[e.type]=(m[e.type]||0)+1,m),{})
 const athleteStats=id=>{const es=(draft.events||[]).filter(e=>e.playerId===id);return {goals:es.filter(e=>e.type==='Golo').length,assists:es.filter(e=>e.type==='Assistência').length,shots:es.filter(e=>e.type.startsWith('Remate')).length,rec:es.filter(e=>['Recuperação','Roubo','Interceção'].includes(e.type)).length,loss:es.filter(e=>e.type==='Perda').length}}
 return <div className="gamesWorkspace v221Games"><section className="card gameEditor"><div className="paneTitle"><div><h2>Estatística de Jogo e Desempenho</h2><small>{editingId?'A editar jogo guardado':'Registo preenchível · tudo editável'}</small></div><BarChart3/></div>
 ...
 <div className="sectionTitleRow"><div><h3>Eventos do jogo</h3><small>Minuto, parte, ação, atleta e zona são editáveis.</small></div><Target/></div>
 <div className="customAction"><input placeholder="Nova ação personalizada…" value={customType} onChange={e=>setCustomType(e.target.value)} onKeyDown={e=>e.key==='Enter'&&addCustomType()}/><button onClick={addCustomType}><Plus/>Criar ação</button></div>
 <div className="eventEntry v221Event"><input type="number" min="0" placeholder="Min." value={event.minute} onChange={e=>setEvent({...event,minute:e.target.value})}/><select value={event.period} onChange={e=>setEvent({...event,period:e.target.value})}><option>1ª</option><option>2ª</option><option>Prolong.</option></select><select value={event.type} onChange={e=>setEvent({...event,type:e.target.value})}>{types.map(x=><option key={x}>{x}</option>)}</select><select value={event.playerId} onChange={e=>setEvent({...event,playerId:e.target.value})}><option value="">Equipa / sem atleta</option>{eligible.map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</select><input placeholder="Zona/campo" value={event.zone} onChange={e=>setEvent({...event,zone:e.target.value})}/><input placeholder="Nota" value={event.note} onChange={e=>setEvent({...event,note:e.target.value})}/><button className="primary" onClick={addEvent}><Plus/>{tr.addEvent}</button></div>
 <div className="eventList editableEvents">{(draft.events||[]).map(ev=><div className="gameEvent editEvent" key={ev.id}><input className="mini" type="number" min="0" value={ev.minute||''} onChange={e=>updateEvent(ev.id,'minute',e.target.value)}/><select value={ev.period||'1ª'} onChange={e=>updateEvent(ev.id,'period',e.target.value)}><option>1ª</option><option>2ª</option><option>Prolong.</option></select><select value={ev.type} onChange={e=>updateEvent(ev.id,'type',e.target.value)}>{types.map(x=><option key={x}>{x}</option>)}</select><select value={ev.playerId||''} onChange={e=>updateEvent(ev.id,'playerId',e.target.value)}><option value="">Equipa</option>{eligible.map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</select><input value={ev.zone||''} placeholder="Zona" onChange={e=>updateEvent(ev.id,'zone',e.target.value)}/><input value={ev.note||''} placeholder="Nota" onChange={e=>updateEvent(ev.id,'note',e.target.value)}/><button className="dangerLite" onClick={()=>setDraft({...draft,events:draft.events.filter(x=>x.id!==ev.id)})}><X/></button></div>)}</div>
 <div className="sectionTitleRow"><div><h3>Estatística coletiva</h3><small>Preencha manualmente ou use os eventos como referência.</small></div><Trophy/></div>
 <div className="teamStatsGrid"><Field label="Posse %"><input type="number" min="0" max="100" value={draft.teamStats?.possession||''} onChange={e=>setTeam('possession',e.target.value)}/></Field><Field label="Remates"><input type="number" min="0" value={draft.teamStats?.shots||''} onChange={e=>setTeam('shots',e.target.value)}/></Field><Field label="Enquadrados"><input type="number" min="0" value={draft.teamStats?.shotsOnTarget||''} onChange={e=>setTeam('shotsOnTarget',e.target.value)}/></Field><Field label="Recuperações"><input type="number" min="0" value={draft.teamStats?.recoveries||''} onChange={e=>setTeam('recoveries',e.target.value)}/></Field><Field label="Perdas"><input type="number" min="0" value={draft.teamStats?.losses||''} onChange={e=>setTeam('losses',e.target.value)}/></Field><Field label="Cantos"><input type="number" min="0" value={draft.teamStats?.corners||''} onChange={e=>setTeam('corners',e.target.value)}/></Field><Field label="Faltas"><input type="number" min="0" value={draft.teamStats?.fouls||''} onChange={e=>setTeam('fouls',e.target.value)}/></Field><Field label="Bolas paradas"><input type="number" min="0" value={draft.teamStats?.setPieces||''} onChange={e=>setTeam('setPieces',e.target.value)}/></Field><Field label="Transições"><input type="number" min="0" value={draft.teamStats?.transitions||''} onChange={e=>setTeam('transitions',e.target.value)}/></Field><Field label="Ligações no pivô"><input type="number" min="0" value={draft.teamStats?.pivotLinks||''} onChange={e=>setTeam('pivotLinks',e.target.value)}/></Field><Field label="Chegadas 2.º poste"><input type="number" min="0" value={draft.teamStats?.secondPost||''} onChange={e=>setTeam('secondPost',e.target.value)}/></Field></div>
 <Field label="Notas coletivas / modelo de jogo"><textarea rows="3" value={draft.teamStats?.notes||''} onChange={e=>setTeam('notes',e.target.value)}/></Field>
 <div className="sectionTitleRow"><div><h3>Desempenho individual</h3><small>Todos os valores e observações podem ser alterados a qualquer momento.</small></div><Users/></div>
 <div className="performanceTable"><div className="perfHead"><b>Atleta</b><b>Nota</b><b>Téc.</b><b>Tát.</b><b>Fís.</b><b>Atit.</b><b>Resumo</b></div>{eligible.map(a=>{const p=draft.performance?.[a.id]||{},st=athleteStats(a.id);return <div className="perfRow" key={a.id}><div><b>{a.name}</b><small>{st.goals} G · {st.assists} A · {st.shots} R · {st.rec} Rec · {st.loss} P</small></div>{['rating','technical','tactical','physical','attitude'].map(k=><input key={k} type="number" min="0" max="10" step="0.5" value={p[k]??''} onChange={e=>setPerf(a.id,k,e.target.value)}/>)}<input value={p.notes||''} placeholder="Observação…" onChange={e=>setPerf(a.id,'notes',e.target.value)}/></div>})}</div>
 <div className="liveTotals"><span><b>{counts['Golo']||0}</b> golos</span><span><b>{counts['Remate']||0}</b> remates</span><span><b>{(counts['Recuperação']||0)+(counts['Roubo']||0)+(counts['Interceção']||0)}</b> recuperações</span><span><b>{counts['Perda']||0}</b> perdas</span></div>
 <Field label={tr.notes}><textarea rows="3" value={draft.notes} onChange={e=>setDraft({...draft,notes:e.target.value})}/></Field><div className="saveGameRow"><button className="primary wideAction" onClick={save}><Save/>{editingId?'Guardar alterações':'Guardar jogo'}</button>{editingId&&<button onClick={cancelEdit}><X/>Cancelar edição</button>}</div></section>
 <aside className="card gamesHistory"><div className="paneTitle"><h3>{tr.history}</h3><small>{games.length}</small></div>{games.slice().reverse().map(g=><div className="gameCard v221GameCard" key={g.id}><div><b>Gruefwiss {g.goalsFor}–{g.goalsAgainst} {g.opponent}</b><small>{g.date||'—'} · {g.competition||'—'} · {(g.events||[]).length} ações</small></div><div className="gameCardActions"><button onClick={()=>editGame(g)}><Pencil/>Editar</button><button className="dangerLite" onClick={()=>{if(window.confirm('Eliminar este jogo?'))setGames(games.filter(x=>x.id!==g.id))}}><Trash2/></button></div></div>)}</aside><SeasonStats tr={tr} athletes={athletes} games={games} sessions={[]} callups={callups}/></div>
}
